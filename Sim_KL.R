...
#install.packages("copula", repos = "https://cloud.r-project.org/", dependencies = TRUE)
...
# betaT1=unname(rq(Y1~W1-1,tau = Qt1)$coef)
# betaT2=unname(rq(Y2~W2-1,tau = Qt2)$coef)


#b=rmvnorm(length(T),rep(0,length(Z[1,])))
# b1=matrix(rep(0,length(T)*p3),nrow = length(T),ncol = p3)
# b2=matrix(rep(0,length(T)*p3),nrow = length(T),ncol = p3)
...
init00=list(
  ...
  L_omega=L_omega,
  # betaT1=as.array(betaT1+rnorm(length(betaT1),0,0.1)),
  # betaT2=as.array(betaT2+rnorm(length(betaT2),0,0.1)),
...
#fit=stan(file = "A_J_KL_PP.stan",data = Data, warmup = 50, iter = 100, init = init0, chains = 2, cores =2, refresh =10) #, control = list(adapt_delta = 0.99))


fit=stan(file = "~/scratch/Sim_KL/A_J_KL_PP.stan",data = Data, warmup = 10000, iter = 20000, init = init0, chains = 2, cores =2, refresh=10)

#summary(fit, pars = c("beta1", "beta2", "theta", "Ktau", "rho", "betaT", "rrho", "sig"), probs = c(0.025, 0.975))$summary
...
Time_Taken=unname(proc.time() - ptm)[3]/(60*60)
Time_Taken

#sink("fit_minTi_20000b1b2.txt",append = FALSE)
#round(summary(fit, pars = c("beta1", "beta2", "theta", "Ktau", "LU", "LV", "rho", "betaT", "rrho", "sig"), probs = c(0.025, 0.975))$summary,digits = 2)
#Time_Taken
#sink()
...
write.table(t(c(E_B1, E_B2, E_T,E_Ktau,c(t(E_betaT)), E_rrho, E_rho, E_sig)),"~/scratch/Sim_KL_20000/Para_C_J.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
write.table(t(c(SD_B1, SD_B2, SD_T,SD_Ktau,c(t(SD_betaT)), SD_rrho, SD_rho, SD_sig)),"~/scratch/Sim_KL_20000/SD_C_J.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
write.table(t(c(CP_B1, CP_B2, CP_T,CP_Ktau,c(t(CP_betaT)), CP_rrho, CP_rho, CP_sig)),"~/scratch/Sim_KL_20000/CP_C_J.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
...
write.table(t(c(E_LU, E_LV)),"~/scratch/Sim_KL_20000/Para_C_LULV.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
write.table(t(c(SD_LU, SD_LV)),"~/scratch/Sim_KL_20000/SD_C_LULV.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
write.table(t(c(CP_LU, CP_LV)),"~/scratch/Sim_KL_20000/CP_C_LULV.txt",append = TRUE, row.names = FALSE, col.names = FALSE)
